...
if constr_succes_state_ens:
    ...
    plot_pdf_panels = False #plot figure panel with pdfs
    plot_colored_corr_matr = False #plot a colored correlation matrix
...
print_NrStDev_from_truth = True #Only for OSSEs, print (for state parameters) posterior minus truth, normalised with posterior standard deviation
...
plot_manual_fitpanels = True #panels of figures, showing obs and model
plot_auto_fitpanels = False  #a panel of figures, showing obs and model. More automated, number of rows and nr of columns are specified by two variables 
plot_enbal_panel = False #plot a figure panel with observations corrected for the energy balnce error using FracH 
...
display_names['CO2mh'] = 'CO2$_{20}$'
disp_units['wCO2'] = 'mg CO$_2$ m$^{-2}$s$^{-1}$'
disp_units['h'] = 'm'
disp_units['CO2mh'] = 'ppm'
disp_units['H'] = 'W m$^{-2}$'
disp_units['LE'] = 'W m$^{-2}$'
...
if plot_manual_fitpanels:
    ...
    plt.rc('font', size=16)       
    plotvars = ['h','CO2mh']  #first the var for 0,0 than 0,1 than 1,0 than 1,1
    unsca = np.ones(len(plotvars)) #a scale for plotting the obs with different units
    for i in range(len(plotvars)):
        if (disp_units[plotvars[i]] == 'g/kg' or disp_units[plotvars[i]] == 'g kg$^{-1}$') and (plotvars[i] == 'q' or plotvars[i].startswith('qmh')): #q can be plotted differently for clarity
            unsca[i] = 1000
...
#    ax[0,0].errorbar(obs_times[plotvars[0]]/3600,unsca[0]*optim.__dict__['obs_'+plotvars[0]],yerr=unsca[0]*optim.__dict__['error_obs_'+plotvars[0]],ecolor='lightgray',fmt='None',label = '$\sigma_{O}$', elinewidth=2,capsize = 0)
    ...
    ax[0,0].plot(priormodel.out.t,unsca[0]*priormodel.out.__dict__[plotvars[0]], ls='dashed', marker='None',color='gold',linewidth = 2.0,label = 'prior',dashes = (4,4))
    ax[0,0].plot(optimalmodel.out.t,unsca[0]*optimalmodel.out.__dict__[plotvars[0]], linestyle='-', marker='None',color='red',linewidth = 2.0,label = 'post')
...
#    ax[0,1].errorbar(obs_times[plotvars[1]]/3600,unsca[1]*optim.__dict__['obs_'+plotvars[1]],yerr=unsca[1]*optim.__dict__['error_obs_'+plotvars[1]],ecolor='lightgray',fmt='None',label = '$\sigma_{O}$', elinewidth=2,capsize = 0)
    ax[0,1].plot(priormodel.out.t,unsca[1]*priormodel.out.__dict__[plotvars[1]], ls='dashed', marker='None',color='gold',linewidth = 2.0,label = 'prior',dashes = (4,4))
    ax[0,1].plot(optimalmodel.out.t,unsca[1]*optimalmodel.out.__dict__[plotvars[1]], linestyle='-', marker='None',color='red',linewidth = 2.0,label = 'post')
    ...
    if 'obs_sca_cf_'+plotvars[1] in state: #plot the obs scaled with the scaling factors (if applicable)
        ax[0,1].plot(obs_times[plotvars[1]]/3600,optimalinput.__dict__['obs_sca_cf_'+plotvars[1]]*unsca[1]*optim.__dict__['obs_'+plotvars[1]], linestyle=' ', marker='o',color = 'red',ms=5,label = 'obs sca')
    ax[0,1].errorbar(obs_times[plotvars[1]]/3600,optimalinput.__dict__['obs_sca_cf_'+plotvars[1]]*unsca[1]*optim.__dict__['obs_'+plotvars[1]],yerr=unsca[1]*measurement_error[plotvars[1]],ecolor='black',fmt='None',label = '$\sigma_{I}$')
    ax[0,1].set_ylabel(display_names[plotvars[1]] +' ('+ disp_units[plotvars[1]] + ')')
    ax[0,1].set_xlabel('Time (h)')
    ax[0,1].legend(loc=0, frameon=True,prop={'size':16})
    
    enbal_corr_H = optim.obs_H + optimalinput.FracH * optim.EnBalDiffObs_atHtimes
#    ax[1,0].errorbar(obs_times['H']/3600,enbal_corr_H,yerr=optim.__dict__['error_obs_H'],ecolor='lightgray',fmt='None',label = '$\sigma_{O}$', elinewidth=2,capsize = 0)
    ax[1,0].errorbar(obs_times['H']/3600,enbal_corr_H,yerr=measurement_error['H'],ecolor='black',fmt='None',label = '$\sigma_{I}$')
    ax[1,0].plot(priormodel.out.t,priormodel.out.H, ls='dashed', marker='None',color='gold',linewidth = 2.0,label = 'prior')
    ax[1,0].plot(optimalmodel.out.t,optimalmodel.out.H, linestyle='-', marker='None',color='red',linewidth = 2.0,label = 'post')
    if use_ensemble:
        if pert_non_state_param and opt_sim_nr != 0:
    ...
    ax[1,0].plot(obs_times['H']/3600,optim.__dict__['obs_'+'H'], linestyle=' ', marker='*',color = 'black',ms=10,label = 'obs ori')
    ax[1,0].plot(obs_times['H']/3600,enbal_corr_H, linestyle=' ', marker='o',color = 'red',ms=10,label = 'obs cor')
    ax[1,0].set_ylabel('H (' + disp_units['H']+')')
    ax[1,0].set_xlabel('Time (h)')   
    enbal_corr_LE = optim.obs_LE + (1 - optimalinput.FracH) * optim.EnBalDiffObs_atLEtimes
#    ax[1,1].errorbar(obs_times['LE']/3600,enbal_corr_LE,yerr=optim.__dict__['error_obs_LE'],ecolor='lightgray',fmt='None',label = '$\sigma_{O}$', elinewidth=2,capsize = 0)
    ax[1,1].errorbar(obs_times['LE']/3600,enbal_corr_LE,yerr=measurement_error['LE'],ecolor='black',fmt='None',label = '$\sigma_{I}$')
    ax[1,1].plot(priormodel.out.t,priormodel.out.LE, ls='dashed', marker='None',color='gold',linewidth = 2.0,label = 'prior')
    ax[1,1].plot(optimalmodel.out.t,optimalmodel.out.LE, linestyle='-', marker='None',color='red',linewidth = 2.0,label = 'post')
    if use_ensemble:
        if pert_non_state_param and opt_sim_nr != 0:
    ...
    ax[1,1].plot(obs_times['LE']/3600,optim.__dict__['obs_'+'LE'], linestyle=' ', marker='*',color = 'black',ms=10,label = 'obs ori')
    ax[1,1].plot(obs_times['LE']/3600,enbal_corr_LE, linestyle=' ', marker='o',color = 'red',ms=10,label = 'obs cor')
    ax[1,1].set_ylabel('LE (' + disp_units['LE']+')')
    ax[1,1].set_xlabel('Time (h)')
    ax[1,1].legend(prop={'size':16},loc=0)
    ...
    ax[1,1].annotate('(d)',
                ...
                textcoords='offset points',
                size=16, fontweight='bold',ha='left', va='top')
    plt.subplots_adjust(left=0.08, right=0.94, top=0.92, bottom=0.10,wspace=0.15)
    plt.savefig('pp_fig_fitpanel_withEnBal.'+figformat, format=figformat)
    plt.rc('font', size=plotfontsize) #reset plot font size
...
if load_second_optim:
    ...
    if load_third_optim:
        if plot_three_optim_man_fitpanel:
            ...
            ax[3,0].set_ylabel('CO$_2$ flux (ppm ms$^{-1}$)')
