...
if use_ensemble:
    nr_of_members = 101 #number of members in the ensemble of optimisations (including the one with unperturbed prior)
    ...
    if est_post_pdf_covmatr:
        nr_bins = 13 #nr of bins for the pdfs
        succes_opt_crit = 2*1.5*1.5 #the chi squared at which an optimisation is considered successfull (lower or equal to is succesfull)
...
priormodinput.gammaq     = -0.003e-3        # free atmosphere specific humidity lapse rate [kg kg-1 m-1]
...
priormodinput.CO2        = 380.     # initial mixed-layer CO2 [ppm]
...
priormodinput.advCO2     = 6.0e-3        # advection of CO2 [ppm s-1]
...
priormodinput.z0m        = 0.100     # roughness length for momentum [m]
...
#################################################################################
state=['h','alpha','sca_sto','wg','gammatheta','gammaq','CO2','advCO2','z0m','z0h','FracH','obs_sca_cf_CO2mh']
obsvarlist=['h','q','H','LE','Tmh','wCO2','CO2mh']#
...
priorinput.obs_sca_cf_CO2mh = 1.0
#####################################################################################
...
###########################################################
    ...
    priorvar['alpha'] = 0.15**2
    priorvar['gammatheta'] = 0.003**2 
    priorvar['gammaq'] = (0.003e-3)**2 
    priorvar['sca_sto'] = 1.5**2
    priorvar['h'] = 200**2
    priorvar['CO2'] = 50**2
    priorvar['advCO2'] = 20/3600**2
    priorvar['z0m'] = 0.1**2
    priorvar['z0h'] = 0.01**2
    priorvar['wg'] = 0.15**2
    priorvar['obs_sca_cf_CO2mh'] = 0.3**2
#    priorvar['obs_sca_cf_Ts'] = 0.4**2
    if 'FracH' in state:
        priorvar['FracH'] = 0.2**2
...
truthinput.z0m = 0.02
truthinput.z0h = 0.02
truthinput.obs_sca_cf_CO2mh = 1.4
...
##################################################################
    ...
    optim.EnBalDiffObs_atHtimes = 0.25 * (truthmodel.out.H[::6] + truthmodel.out.LE[::6] + truthmodel.out.G[::6])
