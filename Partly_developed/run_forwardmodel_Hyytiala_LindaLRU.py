...
if priorinput.calc_sun_shad == True:
    ...
    plt.figure()
    PAR_m2_ground = priormodel.canopy_model.PAR_sun * priormodel.canopy_model.fract_sun + priormodel.canopy_model.PAR_sha * (1 - priormodel.canopy_model.fract_sun)
    plt.plot(PAR_m2_ground,priorinput.z_veglay)
    plt.ylabel('height (m)')
    plt.xlabel('PAR at t = ' + str(priormodel.t) +' (W m-2 ground)')
    plt.legend()
...
plt.figure()
plt.plot(priorinput.lad_veglay,priorinput.z_veglay)
plt.ylabel('height (m)')
plt.xlabel('leaf area density (m²/m³)')     
    
plt.figure()
plt.plot(priormodel.out.Ds_veg[-1,:],priorinput.z_veglay)
plt.ylabel('height (m)')
plt.xlabel('Ds at t = ' + str(priormodel.t) +' (kPa)')   

plt.figure()
plt.plot(priormodel.out.C_CO2_veglayer_ppm[-1,:],priorinput.z_veglay)
plt.ylabel('height (m)')
plt.xlabel('CO2 mixing rat at t = ' + str(priormodel.t) +' (ppm)')   

plt.figure()
plt.plot(1000*priormodel.out.C_COS_veglayer_ppb[-1,:],priorinput.z_veglay)
plt.ylabel('height (m)')
plt.xlabel('COS mixing rat at t = ' + str(priormodel.t) +' (ppt)')

plt.figure()
plt.plot(1e6*priormodel.out.CO2plantflux[-1,:]/priormodel.canopy_model.dz_veglay,priorinput.z_veglay)
plt.ylabel('height (m)')
plt.xlabel('CO2 veg flux at t = ' + str(priormodel.t) +' (µmol/m3/s)')   

plt.figure()
plt.plot(1e12*priormodel.out.COSplantflux[-1,:]/priormodel.canopy_model.dz_veglay,priorinput.z_veglay)
plt.ylabel('height (m)')
plt.xlabel('COS veg flux at t = ' + str(priormodel.t) +' (pmol/m3/s)')
    
canopy_CO2_mean = np.zeros(len(priormodel.out.t))
canopy_COS_mean = np.zeros(len(priormodel.out.t))
for i in range(len(priormodel.out.t)):
    canopy_COS_mean[i] = np.mean(priormodel.out.C_COS_veglayer_ppb[i])
    canopy_CO2_mean[i] = np.mean(priormodel.out.C_CO2_veglayer_ppm[i])
LRU = priormodel.out.wCOSP[:] / priormodel.out.wCO2A[:] * canopy_CO2_mean / canopy_COS_mean
PAR = 0.5 * priormodel.out.Swin
plt.figure()
plt.plot(priormodel.out.t,LRU,label = 'whole canopy, average mix rat'  )
LRU = priormodel.out.wCOSP[:] / priormodel.out.wCO2A[:] * priormodel.out.CO2 / priormodel.out.COS
PAR = 0.5 * priormodel.out.Swin
plt.plot(priormodel.out.t,LRU,label = 'whole canopy, mix layer mix rat'  )
plt.xlabel('time')
plt.ylabel('LRU (-)')
plt.legend()
...
PAR = 0.5 * priormodel.out.Swin
plt.figure()
plt.plot(priormodel.out.t,LRU,label = 'whole canopy'  )
plt.xlabel('time')
plt.ylabel('LRU (-)')
toplayerco2flux_ppm_m_per_s = priormodel.out.CO2plantflux[:,-1] * 1.e6 / priormodel.rho * priormodel.mair * 0.001 #from mol / m2/s to ppm m s-1 
toplayerCOSflux_ppb_m_per_s = priormodel.out.COSplantflux[:,-1] * 1.e9 / priormodel.rho * priormodel.mair * 0.001 #ppb m s-1
LRU_cantop = toplayerCOSflux_ppb_m_per_s[:] / toplayerco2flux_ppm_m_per_s[:] * priormodel.out.C_CO2_veglayer_ppm[:,-1] / priormodel.out.C_COS_veglayer_ppb[:,-1]
plt.plot(priormodel.out.t,LRU_cantop,label = 'canopy top' )
LRU_avg_mixrat = priormodel.out.wCOSP[:] / priormodel.out.wCO2A[:] * canopy_CO2_mean / canopy_COS_mean
plt.plot(priormodel.out.t,LRU_avg_mixrat,label = 'whole canopy, average mix rat'  )
plt.xlabel('time')
plt.ylabel('LRU (-)')
plt.legend()

LRU_height = np.zeros(priormodel.canopy_model.nveglay)
for lay in range(priormodel.canopy_model.nveglay):
    layerCOSflux_ppb_m_per_s = priormodel.out.COSplantflux[-1,lay] * 1.e9 / priormodel.rho * priormodel.mair * 0.001 #ppb m s-1
    layerco2flux_ppm_m_per_s = priormodel.out.CO2plantflux[-1,lay] * 1.e6 / priormodel.rho * priormodel.mair * 0.001 #from mol / m2/s to ppm m s-1 
    LRU_height[lay] = layerCOSflux_ppb_m_per_s / layerco2flux_ppm_m_per_s * priormodel.out.C_CO2_veglayer_ppm[-1,lay] / priormodel.out.C_COS_veglayer_ppb[-1,lay]
plt.figure()
plt.scatter(LRU_height,priorinput.z_veglay)
plt.axvline(LRU_avg_mixrat[-1],color ='red')
plt.xlabel('LRU (-)')
plt.ylabel('height (m)')
...
plt.figure()
plt.scatter(PAR,LRU_cantop)
plt.xlabel('PAR (W/m2)')
plt.ylabel('LRU cantop(-)')

plt.figure()
plt.plot(priormodel.out.t,toplayerco2flux_ppm_m_per_s,label = 'canopy top' )
plt.xlabel('time')
plt.ylabel('CO2plantflux top (ppm m/s)')

plt.figure()
plt.plot(priormodel.out.t,toplayerCOSflux_ppb_m_per_s,label = 'canopy top' )
plt.xlabel('time')
plt.ylabel('COSplantflux top (ppb m/s)')

plt.figure()
plt.plot(priormodel.out.t,priormodel.out.C_CO2_veglayer_ppm[:,-1],label = 'canopy top' )
plt.xlabel('time')
plt.ylabel('CO2 top (ppm)')

plt.figure()
plt.plot(priormodel.out.t,priormodel.out.C_COS_veglayer_ppb[:,-1],label = 'canopy top' )
plt.xlabel('time')
plt.ylabel('COS top (ppb)')