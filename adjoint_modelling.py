...
class adjoint_modelling:
    ...
    def __init__(self,model,write_to_file=False,use_backgr_in_cost=False,imposeparambounds=False,state=None,Optimfile='Optimfile.txt',Gradfile='Gradfile.txt'):                         
        ...
        self.nr_of_restarted_sim = 0 #the simulation nr at which the optimisation has restarted from the end (if applicable). 
        if self.write_to_f:
...
#                            cost = 0
        ...
        if self.sim_nr > 15 and (self.sim_nr - self.nr_of_restarted_sim > 9): #second condition to prevent termination immediately after restart
            too_small_diff = True
            ...
            if too_small_diff == True:
                ...
                self.nr_of_restarted_sim = self.sim_nr
